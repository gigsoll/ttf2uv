...
class SubtableFormat4(Subtable):
    def __init__(self, binary_data: bytes):
        ...
        constants: SubtableFormat4Constants = self.prepare_constants()

        # searchRange, entrySelector, and rangeShift are scipped, each UINT16
        seg_count = constants.seg_count
        self.end_codes: list[bytes] = self.read_array(
            constants.endcodes_start, seg_count
        )
        self.start_codes: list[bytes] = self.read_array(
            constants.start_codes_start, seg_count
        )
        self.id_delta: list[bytes] = self.read_array(constants.deltas_start, seg_count)
        self.id_range_offset: list[bytes] = self.read_array(
            constants.id_range_offset_start, seg_count
        )
        self.glyf_index_array: list[bytes] = self.read_array(
